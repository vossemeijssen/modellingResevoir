...
def FVM_Sw_t_2(p,Sw,c,N,M,hx,hy):
    Swt = np.zeros(N*(M+1) + (N+1)*M)
    ...
    for j in range(M+1):
        for i in range(N):
            ...
            if j == 1:
                ...
                boundary3 = hx*(l_w(Sw[index_Sw_xP + N + 1],c)+l_w(Sw[index_Sw_xP + N + 2],c))/2*(p[index_p + N    ]-p[index_p    ])/hy
                boundary4 = hx*(l_w(Sw[index_Sw_xP        ],c)+l_w(Sw[index_Sw_xP     + 1],c))/2*(p[index_p - N    ]-p[index_p])/2/hy
            else:
                boundary3 = hx*(l_w(Sw[index_Sw_xP + N + 1],c)+l_w(Sw[index_Sw_xP + N + 2],c))/2*(p[index_p + N]-p[index_p    ])/2/hy
                boundary4 = hx*(l_w(Sw[index_Sw_xP        ],c)+l_w(Sw[index_Sw_xP     + 1],c))/2*(p[index_p - N    ]-p[index_p])/2/hy
